...
void FormPostprocessing::loadResultData(QString caseFilePath)
{
...
void FormPostprocessing::on_pushButton_clicked()
{
	...
	caseFilePath = QFileDialog::getOpenFileName(this, tr("加载后处理数据"), "",
		tr("OpenFOAM 文件 (*.foam);;"));

	if (caseFilePath == "") return;

	loadResultData(caseFilePath);
}
